...
【選手発言の除外パターン】
以下は選手本人（や監督・関係者本人）の発言なので絶対に抽出しない：
- 「〜と喜んだ」「〜と悔しがった」「〜と笑顔で語った」
- 「〜と意気込みを語った」「〜と抱負を語った」
- 一人称・自己視点の発言全般（自分の心境・方針・目標・感想）。例：「楽しんでいます」「気にしていないです」「〜をモットーにしている」「頑張りたい」「自信がある」など。話者が自分自身について語る内容はスカウトの選手評価ではない。

【スカウトコメントの内容定義（厳守）】
- スカウトコメントとは「スカウト等の第三者が、対象選手を客観的に評価・分析する発言」に限る（例：球速・球種・技術・体格・走力・将来性・他選手との比較など）。
- 選手の心境・方針・目標・感想（一人称の自己言及）は、たとえ「」で囲まれていてもスカウト評価ではないので抽出しない。

【「視察」と「発言」の区別（厳守）】
- 「スカウトが視察に訪れた」「多くのスカウトが来場した」「○球団のスカウトが見つめた」などは“行動の記述”であって発言ではない。これを根拠にスカウトコメントを作ってはいけない。
- 抽出してよいのは、スカウト本人が主語で選手を評価した引用（例：「○○スカウトは『…』と評価した」）のみ。
- 各発言について、引用の直前の主語を必ず確認する。主語が選手本人（「○○選手は」「○○は」）であれば、たとえ前後に「スカウト」の語があっても抽出しない。
...
- 記事は必ず Markdown の見出し（##）を使って構造化する。プレーンテキストの段落だけを並べてはいけない。
  ...
  4. スカウト評価一覧（`## スカウト評価` という見出しを付ける）
     - summary_json の scout_comments に含まれるスカウトコメントを、確認できたものは漏れなく箇条書きで一覧化する。読者がスカウトの声を一目で見渡せるようにする。
     - 各項目は「- 【球団名】スカウト名「コメント全文」」の形式にする。
       - scout_name がある場合は必ずスカウト名を明記する（例: 「- 【巨人】田中スカウト「完成度は発展途上だが素材は一級品」」）。データに scout_name があるのに省略してはいけない。
       - scout_name が無い（null/空）場合は「- 【球団名】「コメント全文」」とする（例: 「- 【オリックス】「先発の柱になれるポテンシャルを感じる」」）。
       - 球団名が不明な場合はスカウト名のみ、両方不明なら「- 「コメント全文」」とする。
     - 本記の中で引用済みのコメントも、ここで改めて一覧としてまとめてよい。
     - コメント文・球団名・スカウト名は creative に変えず、scout_comments の内容を正確に転記する（要約・改変しない）。
     - scout_comments が空、またはスカウトコメントが1件も無い場合は、このセクションごと省略する。

  5. 結び（`## 今後の見どころ` など内容に応じた見出しを付けた1段落）
...
キー:
- title: 記事タイトル。ドラフト専門メディアで実際に使われている文体を厳密に踏襲する。以下のルールをすべて満たすこと。

  (A) 構造: 「【カテゴリ】第1節、第2節（、第3節）」——読点「、」で区切った2〜3節で組み立てる。1節だけの短い要約タイトルにしない。
     - 第1節: この記事で最も具体的で他の記事と入れ替えのきかない事実（誰が何をしたか）。ここに主役と主要な数値を置く。
     - 第2節: 裏付けとなる別の数値・成績、スカウトや球団の反応、本人コメント、対比となる事実のいずれか。
     - 第3節(任意): 今後の展望や位置づけ。

  (B) 冒頭の【カテゴリ】タグ（必須。summary_jsonのtopic_type・main_player・eventsから判断）:
     - 高校の選手・チームが主役 → 【高校野球】 / 大学 → 【大学野球】 / 社会人・企業チーム → 【社会人野球】
     - 侍ジャパン（U18・大学日本代表など）の活動が主題 → 【侍ジャパン】
     - topic_type が scout_meeting、または球団のスカウト会議が主題 → 【スカウト会議】
     - 特定球団の指名方針・ドラフト戦略が主題 → 【巨人】【阪神】【日本ハム】のように球団名そのものをタグにする
     - どれにも当てはまらないドラフト全般の話題 → 【ドラフト】

  (C) 選手の書き方: 「所属・選手名＋肩書」。所属と名前は「・」で繋ぐ（例: 早大・阿部葉太選手、横浜高・織田翔希投手）。肩書は投手なら「〜投手」、捕手なら「〜捕手」、その他の野手は「〜選手」で必ず付ける。
     - 選手名の前に、データにある事実だけで作った「冠（枕詞）」を付けると良い。例:「最速155キロ右腕・」「152キロ左腕・」「1番・」「入社4年目の」「高校通算18本塁打の大砲・」「甲子園準V・」「プロ注目・」「ドラフト候補左腕・」。冠は毎回付けなくてよいが、データにあるなら積極的に使う。

  (D) 数値を必ず1つ以上入れる（無い場合のみ省く。創作は禁止）。球速・イニングと成績・打撃成績・本塁打数・視察球団数や人数・身長・連続記録など。算用数字を使い、「3回無安打5奪三振」「4打数4安打」「29回連続無失点」「2打席連発、3安打5打点」のように成績はコンパクトに連記する。

  (E) 表現の引き出し（毎回同じ型にならないよう、記事内容に合うものを選ぶ）:
     - 本人・関係者の短い発言を末尾に「」で置く（20字以内。scout_commentsやeventsに実在する発言のみ）。例:「…、150キロ左腕「指名されることに照準を置きたい」」
     - 逆接「〜も」で対比を作る。例:「156キロ記録も大学ジャパンに2回6失点、日米18球団が視察し評価は変わらず」
     - スカウト・球団名を出して評価を伝える。例:「巨人・中日スカウト幹部が評価」「阪神スカウト評価で3度目の正直へ」
     - 文末の型は話題ごとに変える:「〜へ」「〜に貢献」「〜が評価」「〜目指す」「〜が視察」「〜変わらず」「〜か」、体言止め、「〜一覧」など。同じ締め方を機械的に繰り返さない。

  (F) 禁止事項:
     - 「怪物」「熱視線」「驚愕」「衝撃」のような煽り語・大げさな形容は使わない。引きは具体的な事実と数値で作る。事実として確認できる「史上初」「劇的」「自己最速更新」などは可。
     - 「〜が注目される」「〜が話題に」のような、中身のない抽象的な締めで終わらない。
     - 「好投」「活躍」「アピール」だけで内容を済ませない（何回何失点、何打数何安打まで書く）。

  (G) 長さは【】を含めて35〜60字程度（目安50字前後）。

  (H) お手本（この文体・情報密度を再現する）:
     - 【高校野球】沖縄尚学・末吉良丞投手がプロ志望届提出、150キロ左腕「指名されることに照準を置きたい」
     - 【社会人野球】最速155キロ右腕・Honda鈴鹿の川原嗣貴投手が都市対抗初登板で3回無安打5奪三振、予選7試合無失点
     - 【高校野球】横浜高・織田翔希投手が156キロ記録も大学ジャパンに2回6失点、日米18球団が視察し評価は変わらず
     - 【大学野球】早大・阿部葉太選手が巨人3軍戦で初回先頭打者アーチ、1年生スラッガーが秋へ
     - 【スカウト会議】日本ハムがスカウト会議で指名候補を約90人に絞る、大分商・平田玲翔投手の評価上昇し北海道関係は10人未満
     - 【スカウト会議】広島がスカウト会議で高校生19選手を評価、横浜高・織田翔希投手を「大学・社会人含めトップクラス」と絶賛
     - 【社会人野球】1番・JFE西日本の原俊太選手が4打数4安打、V候補トヨタ自動車を打ち破る
     - 【大学野球】青学大・渡部海捕手、プロ注目・横浜の織田翔希投手から2安打2打点「大学生として負けられない試合だった」
     - 【高校野球】仙台育英・有本豪琉選手が新チーム初戦で2打席連発、3安打5打点でライバル東北高にコールド勝ち
     - 【巨人】ドラフト1位は投手か打者か、新監督の意向が大きく影響か
